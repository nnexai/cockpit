...
import { CommentDrafts, InlineCommentDrafts } from "../context/CommentDrafts";
...
export function ReviewViewer({ client, presentation, value, onChange, onTerminalView, onRequestControl }: {
      ...
      renderFile={(review, diff, content) => {
        ...
        return <CommentDrafts client={client} presentation={presentation} root={root} sourceIdentity={review.source_id} sourceKind="review" reviewCapture={reference} path={path} document={document} selection={range} mode="source" editorState={value.commentEditor} onEditorStateChange={editorChange} invalidationGeneration={invalidation}>
          {(drafts, actions) => <>
            ...
            {mode === "source" && text !== null ? <SourceLines text={text} state={{ rootId: root.root_id, path, mode: "source", revision, selectionStart: range?.start ?? null, selectionEnd: range?.end ?? null, scrollTop: 0 }} onSelect={(start, end) => setSelection({ fileId: diff.file.file_id, side, start, end })} onScroll={() => undefined} commentDrafts={drafts.filter(draft => draft.file_ref.review?.file_id === diff.file.file_id && draft.file_ref.review.side === side)} commentActions={actions} /> : content((shown, oldLine, newLine) => <>{(["old", "new"] as const).map(anchorSide => {
              const line = anchorSide === "old" ? oldLine : newLine;
              return line === null ? null : <InlineCommentDrafts key={anchorSide} line={line} drafts={drafts.filter(draft => draft.file_ref.review?.file_id === shown.file.file_id && draft.file_ref.review.side === anchorSide)} actions={actions} />;
            })}</>)}
          </>}
        </CommentDrafts>;
      }} />
