      ...
      renderFile={(review, diff, content, loadSourcePage) => {
        ...
        return <CommentDrafts key={commentIdentity} client={client} presentation={presentation} root={root} sourceIdentity={review.source_id} sourceKind="review" reviewCapture={reference} path={path} document={document} selection={range} mode="source" editorState={value.commentEditor} onEditorStateChange={editorChange} invalidationGeneration={invalidation} inlineEditor showToolbar={false} onCommentStatusChange={(next) => updateCommentStatus(commentIdentity, review.comparison, next)} onEditorDismissed={restoreDiffFocus}>
          {(drafts, actions, renderInlineEditor) => {
            ...
            return <>
            {mode === "source" ? <div className="review-source-controls"><strong title={path}>{path.slice(path.lastIndexOf("/") + 1)}</strong><span>{range ? `lines ${Math.min(range.start, range.end)}–${Math.max(range.start, range.end)}` : ""}</span><div className="viewer-segmented" role="group" aria-label="Source side"><button type="button" aria-pressed={side === "old"} disabled={diff.old_source === null && !diff.old_source_truncated} onClick={() => setSelection({ fileId: diff.file.file_id, side: "old", start: 1, end: 1 })}>Old</button><button type="button" aria-pressed={side === "new"} disabled={diff.new_source === null && !diff.new_source_truncated} onClick={() => setSelection({ fileId: diff.file.file_id, side: "new", start: 1, end: 1 })}>New</button></div></div> : null}
