...
const annotationIconPaths = {
  ...
  capture: "M2.5 5h2l1-2h5l1 2h2v8h-11zM8 6.5a2.5 2.5 0 1 0 0 5 2.5 2.5 0 0 0 0-5Z",
  captureAnyway: "M2.5 5h2l1-2h5l1 2h2v8h-11zM8 6.5v3m0 2v.1",
...
export function BrowserPane({ client, target, viewport, visible = true, presentation = "split", clientId, inputActive = true, onInteractionFocus, onFeedback, onHide, onBackToTerminals, className }: BrowserPaneProps) {
  ...
  return <section className={["browser-pane", `browser-pane-${status}`, `browser-tool-${tool}`, className].filter(Boolean).join(" ")} aria-label="Browser view">
    ...
    <div className="browser-annotation-toolbar" role="toolbar" aria-label="Annotation tools">
      ...
      <button type="button" disabled={!draft || !frame} aria-label="Capture" title="Capture annotations" onClick={() => void capture(false)}><AnnotationIcon name="capture" /></button>
      {draft?.freshness === "review_required" ? <button type="button" disabled={!frame} aria-label="Capture as shown" title="Capture current view as shown" onClick={() => void capture(true)}><AnnotationIcon name="captureAnyway" /></button> : null}
      <button type="button" aria-label="Feedback" title="Send browser feedback" onClick={onFeedback}><AnnotationIcon name="feedback" /></button>
    </div>
