...
export type SessionAction =
  ...
  | { type: "attachment/sequence-error"; epoch: number; sessionId: string; paneId: string; streamId: string; message: string };
  ...
  switch (action.type) {
    ...
    case "attachment/message": {
      ...
      if (message.type === "frame") {
        ...
        if (old.terminalSequence !== null && sequence !== old.terminalSequence + 1n) {
          return {
            ...state,
            attachments: {
              ...state.attachments,
    ...
    case "attachment/error":
      if (!current(state, action.epoch, action.sessionId)) return state;
      if (attachment(state, action.paneId).streamId !== action.streamId) return state;
