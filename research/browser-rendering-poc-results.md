...
A fresh Chromium capability matrix tested VP8, VP9, AV1, multiple H.264 profiles, and HEVC at 640×360, 1024×720, 1280×720, and 1920×1080; 30 and 60 fps; realtime and default latency modes; bundled and branded Chrome; headed and unified-headless modes; and `--enable-features=VaapiVideoEncoder,VaapiVideoDecoder --use-gl=egl`. No candidate returned `VideoEncoder.isConfigSupported(... hardwareAcceleration: 'prefer-hardware')` with `supported: true`. Some H.264 profiles returned `supported: true` only for `no-preference`, which is not evidence of hardware encoding. HEVC was unsupported.
