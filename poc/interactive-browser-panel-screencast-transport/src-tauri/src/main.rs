...
impl HelperProcess {
    fn spawn() -> Result<Self, String> {
        ...
        let pending = Arc::new(Mutex::new(HashMap::<
            u64,
            mpsc::SyncSender<Result<Value, String>>,
        >::new()));
        ...
        thread::spawn(move || {
            ...
            while let Ok(read) = reader.read_line(&mut line) {
                ...
                // directly from the helper to the webview over its loopback
                // binary websocket, never through Tauri events or this reader.
    ...
    fn request(
        ...
        let response = match timeout {
            Some(limit) => receiver
                .recv_timeout(limit)
                .map_err(|_| "browser helper response timed out".to_string())
...
fn browser_self_test_report(
    ...
    if success {
        eprintln!("interactive-browser-panel-transport-poc self-test passed: {detail}");
    } else {
        eprintln!("interactive-browser-panel-transport-poc self-test failed: {detail}");
    }
    Ok(())
}
...
fn main() {
    tauri::Builder::default()
        .setup(|app| {
            eprintln!("interactive-browser-panel-transport-poc ready");
