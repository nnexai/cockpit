...
let snapshot; let socket; let reconnect; let decoder; let decoderWidth = 0; let decoderHeight = 0; let decoding = []; let newestSequence = 0; let inputQueue = Promise.resolve(); let pendingPointerMove; let pointerMoveScheduled = false; let selfTestStarted = false;
...
function failDecoder(error) { const detail = error instanceof Error ? error.message : String(error); setStatus(`VideoDecoder failed: ${detail}`, 'error'); emptyState.textContent = `Native WebKit VideoDecoder self-test failed: ${detail}`; emptyState.hidden = false; try { decoder?.close(); } catch {} decoder = undefined; }
function ensureDecoder(packet) {
  ...
  decoder = new VideoDecoder({ output(frame) { const sequence = decoding.shift(); try { canvas.width = frame.displayWidth || frame.codedWidth; canvas.height = frame.displayHeight || frame.codedHeight; context.drawImage(frame, 0, 0, canvas.width, canvas.height); newestSequence = Math.max(newestSequence, sequence || 0); emptyState.hidden = true; if (sequence) acknowledge(sequence); } finally { frame.close(); } }, error: failDecoder });
...
function receivePacket(event) {
  try { const packet = parsePacket(event.data); if (packet.sequence <= newestSequence) { acknowledge(packet.sequence); return; } ensureDecoder(packet); decoding.push(packet.sequence); decoder.decode(new EncodedVideoChunk({ type: packet.keyframe ? 'key' : 'delta', timestamp: packet.timestamp, data: packet.bytes })); } catch (error) { failDecoder(error); socket?.close(); }
}
function connect(streamUrl) {
  ...
  active.addEventListener('close', () => { if (socket !== active) return; try { decoder?.close(); } catch {} decoder = undefined; decoderWidth = 0; decoderHeight = 0; decoding = []; reconnect = setTimeout(() => { if (snapshot?.streamUrl) connect(snapshot.streamUrl); }, 500); });
}
...
async function waitForDecodedSequenceAfter(sequence, timeoutMs = 8_000) { const deadline = performance.now() + timeoutMs; while (newestSequence <= sequence) { if (performance.now() > deadline) throw new Error(`no decoded MediaStream/WebCodecs frame arrived after sequence ${sequence}`); await new Promise((resolve) => setTimeout(resolve, 25)); } }
async function selfTest() { if (selfTestStarted) return; selfTestStarted = true; try { if (!window.VideoDecoder) throw new Error('native WebKit does not provide VideoDecoder'); await waitForDecodedSequenceAfter(0); const beforeReload = newestSequence; applySnapshot(await command('browser_reload')); await waitForDecodedSequenceAfter(beforeReload); const state = await inspect(); if (!state.title.includes('Interactive MediaStream Fixture')) throw new Error('reload did not preserve fixture state'); await command('browser_self_test_report', { success: true, detail: 'MediaStream getDisplayMedia → realtime VP8 VideoEncoder → bounded WebSocket → native VideoDecoder rendered frames before and after reload' }); } catch (error) { const detail = `MediaStream/WebCodecs self-test failed: ${error.message || error}`; failDecoder(error); await command('browser_self_test_report', { success: false, detail }).catch(() => {}); } }
