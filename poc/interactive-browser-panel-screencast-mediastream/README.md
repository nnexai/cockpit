...
The first hardware matrix was inconclusive: it used the obsolete `VaapiVideoEncoder` flag and omitted `bitrateMode`, which defaults WebCodecs to variable bitrate. A corrected headed physical-GPU matrix using Chromium's current `--enable-features=AcceleratedVideoEncoder` path and `bitrateMode: 'constant'` reported only AV1 (`av01.0.04M.08`) `prefer-hardware` support at 30 fps across the tested resolutions; VP8, VP9, and H264 remained unsupported. Direct VA-API probing found the same Mesa driver can encode AV1 but exposes no H264/VP8 encode profile and no VP9 encode entrypoint.

That capability result is not end-to-end proof. Actual AV1 `configure`/encode/flush still failed with `OperationError: Encoder initialization error` in both bundled and branded Chrome 153 probes, so this POC intentionally remains VP8 with `no-preference` rather than switching codecs on a capability query alone. Chromium's current Linux VA-API path is experimental and configuration-dependent; see the [Chromium VA-API notes](https://chromium.googlesource.com/chromium/src/+/refs/heads/main/docs/gpu/vaapi.md), [AV1 VA-API delegate](https://raw.githubusercontent.com/chromium/chromium/main/media/gpu/vaapi/av1_vaapi_video_encoder_delegate.cc), and the [AMD/libva workaround revert](https://chromium.googlesource.com/chromium/src/media/%2B/522e9b8c488c355e1f153e11a77ecd159fe5b179).
