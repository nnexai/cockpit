...
A first Chromium capability matrix was inconclusive: it used the obsolete `--enable-features=VaapiVideoEncoder,VaapiVideoDecoder` spelling and omitted `bitrateMode`, so WebCodecs defaulted to variable bitrate. A corrected headed physical-GPU matrix with `--enable-features=AcceleratedVideoEncoder --ignore-gpu-blocklist --disable-gpu-driver-bug-workaround --use-gl=angle --use-angle=gl` and `bitrateMode: 'constant'` reported only AV1 (`av01.0.04M.08`) as `prefer-hardware` at 30 fps across the tested 640×360 through 1920×1080 resolutions; no 60-fps AV1, VP8, VP9, or H.264 candidate passed. Chromium's VA-API implementation rejects variable bitrate for non-H.264 codecs, so the POC's omitted mode was an independent gating error.

The native Tauri/WebKit probe reported `prefer-hardware` decoder eligibility for VP8, VP9, AV1, and several H.264 profiles, but this is decoder support only and does not establish actual hardware execution. The browser encoder remains the limiting side. H.264 would additionally require carrying the encoder's AVCC decoder description in the packet stream, so no codec swap is justified by this probe.

The corrected capability query still was not end-to-end proof. Actual AV1 `configure`/encode/flush returned `OperationError: Encoder initialization error` in both bundled Chrome for Testing `153.0.8010.12` and branded Chrome `153.0.8010.36`; no encoded chunk was produced. `LIBVA_TRACE` showed Mesa Gallium `26.1.8` initializing AV1 profile 0 / `VAEntrypointEncSlice` and receiving YUV420 plus CQP attributes, then stopped before successful encoder initialization. A raw libva probe against `/dev/dri/renderD128` returned `VAConfigAttribEncAV1Ext1=0x1f`; the libva bitfield places `min_segid_block_size_accepted` in bits 5–12, so this driver reports zero. Chromium's AV1 delegate consumes that field without a zero guard; a February 2026 Chromium workaround defaulted it to 8, then a July 2026 revert removed that workaround as an AMD libva driver bug. This is the concrete local interoperability blocker, while the exact first failing GPU-process call remains opaque behind Chromium's generic error.

The same source path explains why WebRTC would not bypass this blocker: Chromium's WebRTC encoder factory exposes the `VideoEncodeAccelerator` profiles, `RTCVideoEncoder` constructs a constant-bitrate VEA configuration, and the VEA invokes the same AV1 VA-API delegate. WebRTC can use its software AV1 fallback, but changing from WebCodecs to WebRTC does not make this AMD/Mesa hardware path reliable. The POC therefore keeps VP8 software fallback and does not switch codecs from `isConfigSupported()` alone.

Next experiment: rerun the same one-frame AV1 WebCodecs probe after a Mesa/libva or Chromium update, requiring an actual encoded chunk and native decode rather than a support-query result. For diagnosis only, a locally built Chromium with the reverted zero-segment fallback (`seg_size_ = 8`) can distinguish that guard from the later AMD DPB/OBU interoperability failure; it must not replace the production browser path. Until that experiment passes repeated frames without GPU faults, keep the VP8 software fallback.
...
References: [Chrome `getDisplayMedia()` capture guidance](https://developer.chrome.com/docs/extensions/how-to/web-platform/screen-capture), [Chrome `tabCapture` API](https://developer.chrome.com/docs/extensions/reference/api/tabCapture), [MDN WebCodecs](https://developer.mozilla.org/en-US/docs/Web/API/WebCodecs_API), [Chromium VA-API Linux notes](https://chromium.googlesource.com/chromium/src/+/refs/heads/main/docs/gpu/vaapi.md), [Chromium WebCodecs encoder](https://raw.githubusercontent.com/chromium/chromium/main/third_party/blink/renderer/modules/webcodecs/video_encoder.cc), [Chromium AV1 VA-API delegate](https://raw.githubusercontent.com/chromium/chromium/main/media/gpu/vaapi/av1_vaapi_video_encoder_delegate.cc), [AMD/libva AV1 workaround](https://chromium.googlesource.com/chromium/src/%2B/53ae8d905bb1d2895f9d8058a85d7e23ff866d6e), [workaround revert](https://chromium.googlesource.com/chromium/src/media/%2B/522e9b8c488c355e1f153e11a77ecd159fe5b179), and [libva AV1 extension attributes](https://raw.githubusercontent.com/intel/libva/master/va/va_enc_av1.h).
