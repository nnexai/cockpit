...
### 2026-09-06 mouse-input correction

The earlier direct-attach conclusion must not be generalized to every Herdr input surface. Herdr 0.8.2 exposes one-shot `pane.send_keys` and `pane.send_text` operations in addition to the terminal session stream. `pane.send_keys` accepts semantic key tokens, so an SGR report can be emitted as `esc` followed by one printable token per character.

The repository helper `scripts/verify/send_mouse_sgr.py` uses that route. Against the running `MOUSE-FEEDBACK-READY` pane, one invocation sent `ESC[<0;30;9M` followed by `ESC[<0;30;9m`; the pane reported `mouse 2: release button=0 x=30 y=9 wheel=False`, confirming the press/release pair reached the fixture. The user also reports that wheel/scroll events arrive through ordinary xterm.js panes attached to Herdr.

This establishes that stable Herdr 0.8.2 can carry synthetic SGR bytes to a pane and that the normal xterm.js scroll path is usable. It does not by itself prove physical browser/native pointer forwarding, safe app-mode coordinate translation, drag/release semantics, or attachment ownership behavior. Those are separate runtime paths. The likely native-click issue is therefore an interception or routing defect in the current setup, not an established Herdr 0.8.2 impossibility. If native forwarding cannot be repaired, explicit `pane.send_keys`/`pane.send_text` emulation is a viable fallback that must remain ownership-gated and user-visible.

Keep this correction separate from the historical Ghostty/X11 experiment: that experiment remains valid for the physical non-wheel reports it exercised.
...
- Honor documented one-controller ownership and takeover. Track local control intent separately from Herdr focus and DOM focus; external ownership loss clears that intent, preserves observation, and never terminates the Herdr pane/process.
...
- Commands observed during the original read-only inventory: `herdr --version` -> `0.8.2`; `herdr status server` -> running, protocol 20, compatible; `herdr api schema --json`; `herdr api snapshot`; `herdr --skill`; `herdr terminal session observe/control` syntax. No mutating command was run during that original inventory.
- The public docs and upstream source expose terminal stream records and control commands but not a supported high-level Rust library or stable public binary-handshake contract. Cockpit therefore isolates CLI mediation behind `HerdrAdapter`; takeover loss reason mapping and observer fallback are fixture- and integration-tested rather than inferred in the UI.
