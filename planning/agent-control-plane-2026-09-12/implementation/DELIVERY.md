...
| 02 Focus | Focus coordinator and terminal transition gate committed | Busy-output switching and confirmed-target input passed in `cn12` | Yes; final debug install hash verified | — |
...
| 12 Task readiness | GitHub import and direct Context committed | Fresh `cf12` issue #4 setup/create/open/Context/Review workflow passed against the final host and bundle; imported source and retained Review drafts are recorded in `final-workflow-evidence/` | Yes; final debug install hash verified | — |

The final debug install includes all twelve packets and is verified by the fresh `cf12` browser workflow. The requested Tauri and host CLI hashes are recorded in `final-workflow-receipt.json`; the existing user process remains untouched and can load the installed image on its ordinary relaunch.
