...
Space setup shows the primary local repository first, followed by the inputs required for create or open. Put optional context behind one disclosure. Keep an inspectable preview of actual checkout, branch, companion, ownership, and effects before creating resources. Preserve plan expiry, consent, operation identity, partial progress, and cleanup recovery. These are useful information, not decorative warnings to delete.
