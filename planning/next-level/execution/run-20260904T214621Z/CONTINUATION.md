...
[FEATURE_ACCEPTANCE.md](FEATURE_ACCEPTANCE.md) records current behavior, real browser/native/Tea proof, interruption/concurrency repairs and capability limits. [WORKFLOW_ACCEPTANCE.md](WORKFLOW_ACCEPTANCE.md) covers the preceding increment. `CODE_GUIDE.md` is the current module/development map.

All execution checks passed: frontend typecheck/tests88, Rust formatting/workspace check/tests, generated protocol check, browser build and native debug no-bundle build. Core81 and Herdr39 library tests passed; quality fixtures5 passed. Strict quality readiness is inconclusive (exit2): optional coverage/complexity/mutation providers are absent and some integration-test mappings are unresolved.

Excluded/blocked: expanded terminal matrix, stock application mouse/dependent baseline, active PDF/Kitty renderers, parked optional stories, and Tea0.15.1 PRdiff/structured-file/reply subcapabilities. Wiki revision IDs are shortened by Tea. Provider acceptance used the real Tea executable with a controlled GET-only server; production forge permissions/network availability were not tested.
