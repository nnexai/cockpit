...
The campaign is complete only when every required task is `done`, each acceptance criterion has passing evidence on its required surface, all owned changes have commits, and ACCEPT-01 reconciles issues and resources. Missing prerequisites or failed required verification leave affected work open, not done. Stop repeating failed approaches without new evidence, while continuing genuinely independent authorized work. Scope reductions require the user's explicit decision, recorded in the observation ledger and task notes. Do not infer deadlines from example durations or impose default time limits.

One-trigger autonomous execution is defined in [AUTORUN.md](AUTORUN.md). Keep one campaign goal identity and do not require a new user command for each task, but a pause is sticky and never resumes automatically; only explicit user instruction resumes it. The campaign goal completes only after every required acceptance criterion and the final campaign gate pass.
...
Scope implementation and verification as one coherent repair or genuinely independent batch, with owners, protected paths/resources, and an observable acceptance scenario recorded in existing run evidence. Do not create a fresh formal plan for routine verification or understood repair. Preserve original acceptance and task identity; do not create a second status board. The current execution contract in [AUTORUN.md](AUTORUN.md) replaces historical per-task planning/review/verify/commit cadence.
...
Before a repair or independent batch, record scope, owners, acceptance scenario, non-goals, relevant original criteria, prerequisites, exclusive paths/symbols, shared seams, resource locks, and any explicit user limits in existing run evidence. Independent writers require disjoint ownership and fixed interfaces; otherwise serialize them. Inspect relevant briefs, authority, dependency evidence, and current worktree to preserve the contract and pre-existing changes. This concise scoping is not a mandatory planning ceremony or another ledger.

Integration happens once after selected writers settle. One integration owner owns campaign records, protocol/generated files, shared lifecycle boundaries, and unavoidable cross-task contracts. Exclusive owners MAY run cheap focused checks confined to their own work when those checks do not collide with active writers or shared resources. Shared gates wait until concurrent edits settle. Integrate once, run focused relevant static checks and actual-surface acceptance, consolidate observed failures into one repair pass, then rerun affected checks. Commit the verified result or a truthful checkpoint. A read-only reviewer is used only for a named concrete security, data-loss, ownership, or concurrency risk; touching UI, native code, or architecture alone does not mandate a generic review.

For each repaired criterion, after two failed repairs stop editing it; run one materially distinguishing experiment or consult the configured advisor, or checkpoint evidence and stop. Further repair requires new evidence. Classify failures as product defect, automation defect, stale runtime, or missing prerequisite; do not lower acceptance or claim a failed/inconclusive attempt passed. A task remains open until its original acceptance and dependencies pass. A successful batch is not campaign completion; all required tasks, evidence, commits, resource cleanup, and `ACCEPT-01` must still pass the final campaign gate.
...
4. During concurrent writing waves, skip shared builds/tests/formatters/linters/services/commits; exclusive owners may run focused checks confined to their own changes that do not contend on shared writers or resources. Do not mutate user sessions, install live binaries, or change remote issues/MRs unless explicitly assigned a scoped fixture action.
...
For every bug, use a safe reproduction/negative control when feasible, then show whether it no longer triggers. User-reported failures are ground truth: reproduction work establishes measurable acceptance, not whether to believe the report. Source-review candidates must be exercised before claiming a user-visible defect. Preserve passing behavior rather than rewrite a feature merely because the task exists.

For the selected coherent repair or batch, integrate once after edits settle, run focused relevant static checks, and exercise the actual user action through the authoritative response/event to rendered success or actionable failure. Browser proof is required for shared UI; real Linux-native proof is required for native commands/channels/startup/platform input/image decode. macOS-specific behavior is outside this campaign's scope. Compare Herdr semantics in a uniquely named disposable TUI session. Run `skill://playwright-cli` when using that browser workflow, or the harness browser API for direct browser verification.
...
Commit only verified owned changes or a truthful checkpoint, not every task boundary or increment by default. Record resulting real full commit SHAs and evidence paths in `tasks.json` after the commit; checkpoint ledger changes without a self-SHA loop. A task needing more than one bounded repair remains `in_progress`/`verifying` until all original criteria pass and required commits/evidence are recorded. The integrated campaign gate and commit/checkpoint policy are governed by [AUTORUN.md](AUTORUN.md), not a mandatory per-task review-and-commit loop.
...
Append user observations to [OBSERVATIONS](OBSERVATIONS.md) with a stable ID, affected task, and acceptance impact. Queue ordinary feedback; interrupt only a worker whose contract became unsafe or invalid. Feedback does not silently enlarge the current repair. Genuinely independent authorized work may continue. Every in-scope finding must be fixed in an existing task or tracked in an explicit new required task before campaign closure; no invisible follow-up bucket.
