...
For each increment, the orchestrator owns intent, decomposition, integration, verification, commits, and ledger updates:

1. **Select safely.** Resume the owned unfinished increment/task first; do not abandon it merely because `ready` lists other candidates. Otherwise claim a pending task or a genuine disjoint wave whose paths, symbols, sessions, fixtures, ports, displays, and locks are exclusive. Acquire declared locks before dispatch. Record task ID, source baseline, owner, run ID, writable paths/symbols, non-goals, dependencies, prerequisites and resource identities. A `ready` listing is not a claim. Do not take over another live owner; investigate and record deliberate recovery for abandoned ownership.
...
3. **Write the bounded contract.** Before workers edit, put the contract in the run-owned evidence location `runs/<run-id>/<TASK-ID>.md` (or the task's explicitly assigned run record): outcome, preserved criteria, positive and negative checks, required surfaces, baseline, ownership, resources, and stop conditions. Use the supplied evidence template. Detailed observations belong in this record, not in free-form ledger arrays.
4. **Dispatch workers.** Give each worker an exclusive path/symbol contract, relevant authorities and dependency evidence, acceptance criteria, and explicit non-goals. Workers implement only their slice. During a concurrent writing wave workers skip formatters, linters, builds, tests, services, and commits; they do not mutate the active/default Herdr session, user browser profiles, installed binaries, protected `main`, or unauthorized remote fixtures. Workers return changed paths, behavior, risks, proposed focused checks, and any consequential advisor recommendation. A successful worker message is not acceptance.
